...
/**
*
* An array of booleans that can store up to 2^63 elements.
*
* @author Piotr Wendykier (p.wendykier@icm.edu.pl)
*/
public class BooleanLargeArray extends LargeArray {
   	private static final long serialVersionUID = -1629068982609189599L;
	private byte[] data;
    
    public BooleanLargeArray(long length) {
        this.type = LargeArrayType.BOOLEAN;
        this.sizeof = 1;
        if (length <= 0) {
    ...
    public BooleanLargeArray(boolean[] data) {
        this.type = LargeArrayType.BOOLEAN;
        this.sizeof = 1;
    ...
    public void setByte(long i, byte value) {
        if(isLarge()) { 
            Utilities.UNSAFE.putByte(ptr + sizeof * i, value == 0 ? (byte)0 : (byte)1);
        }
        else {
            data[(int)i] = value == 0 ? (byte)0 : (byte)1;
        }
    }
